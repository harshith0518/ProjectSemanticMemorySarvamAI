...
**Latest interview workflow:** Normal Ask separately assesses what is worth remembering and what context an answer needs. Pure questions, greetings, hypotheticals and temporary instructions stay in the current chat. Useful personal/project assertions retain original sources and selected passages; extraction checks new facts, repeats and updates. General answers send no database evidence. For contextual answers, Automatic mode selects learned memories first, attaches only their supporting originals, and uses a bounded lexical original fallback; it does not send every note merely because the collection is small. An explicit inventory request is the deliberate full-small-collection exception. Assessment, learning and answering have separate outcomes and readable timing/token tables. Generated replies are not learned. The local demo defaults to Google `gemini-3.5-flash-lite` at **http://127.0.0.1:18000/**. See [RUN.md](RUN.md) for current checks and limitations; older dated checkpoints below are historical.

**Manual local backup:** setting `KIVI_INFERENCE_PROVIDER=ollama` and `KIVI_OLLAMA_ENABLED=true` before a Compose restart selects host-local `qwen3:4b` for assessment, extraction and answers. The normal setting remains Google. There is no automatic switch between providers. `qwen3-embedding:0.6b` is recorded as the installed future retrieval model, but this version has no persisted vector index or embedding calls; its Automatic retrieval is bounded structured/lexical evidence selection, not a claim of dense semantic search.

**Current state: an integrated synthetic prototype.** The React browser supports typed/pasted notes, import, source inspection, Search, processing, Memories/history, Ask with citations, Correct, Record a change, Forget, feedback diagnosis, sample/corpus import, usage inspection and context-free Private chat. Automatic Ask reviews notes and learned memories, discloses partial retrieval, labels general model knowledge separately and answers calendar dates from the application clock. Missing personal facts and unverified current information are not filled with guesses. API, optional CLI, worker and evaluators share one backend. The approved lifetime ceiling is **1,100 requests / 10,000,000 accounted tokens / $0 paid authorization**, including retries and conservative unknown-usage reservations; prior usage is not reset. Locally entered fictional notes, flexible demo questions and Private direct chat each require a separate default-off operator switch after provider data-terms consent. Private direct sends only the current question to the selected provider: it never reads the database and Kivi does not save its question or reply. A working transport or passing contract test does not establish model accuracy. Full live corpus evaluation remains open.
...
| --- | --- |
| [src/kivi/services.py](src/kivi/services.py) | Composes the shared services used by the API, CLI, worker and evaluators. |
| [src/kivi/turn_assessment.py](src/kivi/turn_assessment.py) | Separates "should this input become a memory candidate?" from "what context may answer it?" before a Normal Ask turn is saved or answered. |
| [src/kivi/conversation.py](src/kivi/conversation.py) and [src/kivi/processing.py](src/kivi/processing.py) | Capture a candidate original, extract exact supported assertions and reconcile claims, repeats and revisions. |
| [src/kivi/retrieval.py](src/kivi/retrieval.py) and [src/kivi/answers.py](src/kivi/answers.py) | Choose bounded evidence, produce cited/contextual or labelled general answers, and release only validated results. |
| [src/kivi/config.py](src/kivi/config.py) and [src/kivi/providers.py](src/kivi/providers.py) | Configure Google as the normal route and the explicit host-local Ollama backup. |
| [frontend/src/lib/use-workspace.ts](frontend/src/lib/use-workspace.ts) | Owns the browser workflow, request lifecycle and screen state. |
| [frontend/src/components/conversation.tsx](frontend/src/components/conversation.tsx) and [frontend/src/components/request-metrics.tsx](frontend/src/components/request-metrics.tsx) | Render Normal/Private conversations, readable citations and the request-metrics cards/tables. |
| [migrations/versions/0006_turn_assessment.py](migrations/versions/0006_turn_assessment.py) | Adds durable, request-bound turn-assessment receipts without changing old source/claim records. |
| [tests/integration](tests/integration) and [tests/browser/workspace.test.mjs](tests/browser/workspace.test.mjs) | Hold the isolated PostgreSQL contracts and real-Chromium workflow checks. |

### Resume in a fresh chat

Start with this README, [todo.md](todo.md), [RUN.md](RUN.md) and the files in the table above. The current code/documentation milestone is complete: the full isolated PostgreSQL suite passed **369 tests in 136.13 seconds**; Ruff, frontend formatting, TypeScript and the Vite production build passed; the rebuilt application reports ready at `http://127.0.0.1:18000/ready` with schema `0006_turn_assessment`.

One acceptance check remains deliberately open. The current aggregate Chromium run completed 18 journeys through **Private cancels pending capture**, then stopped making progress at **Private cancels pending learning**. It was stopped rather than recorded as a pass or failure. Resume by isolating that test under the browser-profile Compose service, determine whether the delay is application behavior or test-harness cleanup, then rerun the complete suite. [RUN.md](RUN.md#browser-acceptance-resume-point) and [todo.md](todo.md#resume-point-for-the-next-chat) contain the exact state and command. The Ollama probe verifies transport only; it is an intentional manual backup, not a completed quality benchmark. Dense/vector retrieval is an explicitly deferred feature, not an implemented dependency.
...
Implementation continues on `dev`; the user controls subsequent merges into `main`. The current interview-hardening implementation is the semantic turn assessment, memory-first bounded retrieval, readable metrics/source labels and explicit Ollama backup described at the top of this README. Start the next implementation chat from [todo.md](todo.md#resume-point-for-the-next-chat) and [RUN.md](RUN.md#browser-acceptance-resume-point), which distinguish verified work from the one unfinished browser acceptance check. The 18-page PDF remains a historical design snapshot. Final Part One documents remain with the applicant and have not been mechanically checked here; the preserved AI-assisted drafts retain separate provenance.
