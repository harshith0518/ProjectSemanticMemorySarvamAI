...
**Latest interview workflow:** Normal Ask automatically preserves user messages, learns useful new assertions, recognizes repeats and keeps revision history for updates before answering. Saving, learning and answering have separate visible outcomes and metrics. Generated replies are not learned. The local demo uses Google `gemini-3.5-flash-lite` at **http://127.0.0.1:18000/**. See [RUN.md](RUN.md) and [the conversation review](eval/reports/interview-conversation-review.json) for current checks and retained model failures; older dated checkpoints below are historical.
