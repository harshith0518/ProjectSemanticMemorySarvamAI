...
Implementation continues on `dev`; the user controls subsequent merges into `main`. The latest instruction approves S10 measurement/reviewer workflow and an explicit S11 corpus/evaluation handoff; the existing bounded synthetic-only NVIDIA allowance is unchanged. The current acceptance record is in [RUN.md](RUN.md#s09-controls-and-synthetic-ask); [todo.md](todo.md) distinguishes implemented workflows from open live-quality gates. The 18-page PDF remains a historical design snapshot. Final Part One documents remain with the applicant and have not been mechanically checked here; the preserved AI-assisted drafts retain separate provenance.
