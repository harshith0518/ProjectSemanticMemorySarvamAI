...
**Latest interview workflow:** Clear public/date questions stay only in the current chat and skip source capture/extraction. Personal and mixed messages preserve their originals, selectively learn useful assertions, recognize repeats and keep revision history. Automatic evidence excludes completed no-memory chat history, while Sources retains the original records. Saving, learning and answering have separate visible outcomes and metrics; generated replies are not learned. The local demo uses Google `gemini-3.5-flash-lite` at **http://127.0.0.1:18000/**. See [RUN.md](RUN.md) and [the routing review](eval/reports/interview-question-routing-review.json) for current checks and limitations; older dated checkpoints below are historical.
