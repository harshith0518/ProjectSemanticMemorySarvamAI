...
**Current state: source workspace plus the S07 memory-processing implementation.** Open [Hey Kivi](http://127.0.0.1:8000/) after Compose starts to import dictations, inspect original sources, request processing and inspect source-linked memories/history. The browser is the primary user surface. S07's backend and browser contracts are tested with explicit deterministic proposals; live extraction remains disabled pending the synthetic-only NVIDIA trial decision, so no model accuracy is claimed. Existing source data remains intact. [RUN.md](RUN.md#s07-memory-processing) explains the controls, operator settings, actual checks and remaining gate. Source-history answers, search improvements and full Correct/Forget remain later work.
...
The S06 readiness audit reviewed all 18 pages against the implemented boundaries. The PDF's DeepSeek labels on pages 9–11 and 16, page 18's pending S03 status, and its implementation-pending footer are historical: S03–S05 are implemented and Kimi K3 is now the response candidate. The diagrams still guide the sequence. S07 processing is now implemented with deterministic acceptance evidence; retrieval, full controls and live model evaluation remain pending. The minimal source workspace was subsequently brought forward from S10 at the user's request. [Page-to-code audit and actual checks](RUN.md#s06-readiness-audit)
...
| Ordinary-user surface | Small HTML/CSS/JavaScript client served by FastAPI | Import, Sources, processing/status, Memories/history and Normal/Private switching; add Ask and complete controls when their backend gates pass. No separate frontend server/build step. |
