...
| `src/kivi/services.py`, `src/kivi/processing.py`, `src/kivi/worker.py`, `src/kivi/models.py`, `0003_memory_processing` migration | One DB-backed worker; lease/retry/replay handling, revision relationships and operation receipts. Reuse S06 accounting rather than creating competing budget stores. Preserve existing source/job/claim rows; model calls occur outside locks, then the whole accepted operation commits under the shared guard. Exact retries do not add evidence. Final columns/migration ordering must match the approved combined scope. |
