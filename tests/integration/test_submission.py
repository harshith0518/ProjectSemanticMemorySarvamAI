...
def test_free_provider_consent_and_no_reviewer_inheritance(monkeypatch, provider):
    ...
    assert not configured.unfamiliar_sources
    assert not configured.private_direct
    monkeypatch.setenv("KIVI_FREE_SYNTHETIC_QUESTIONS_APPROVED", "true")
    monkeypatch.setenv("KIVI_FREE_SYNTHETIC_SOURCES_APPROVED", "true")
    monkeypatch.setenv("KIVI_PRIVATE_DIRECT_APPROVED", "true")
    configured = NvidiaExtractor.from_env()
    assert configured.unfamiliar_questions
    assert configured.unfamiliar_sources
    assert configured.private_direct
