...
The exact long question `what is the capital of USA ? I just want to know the city and in which state it is present` previously failed because request framing triggered the personal-pronoun policy. It now returns Washington, D.C. and explains the federal district with zero notes/memories sent, one answer call and a 2.450 s browser round trip. The planets question also remained unsaved (2.350 s). Personal beverage recall still returned tea with its feeling-good condition and a citation, using 16 eligible notes and 10 learned memories (6.848 s). A new fictional Willow prototype preference produced one scoped memory; repetition returned duplicate with no additional claim. These are eight live calls, separately recorded in [the routing review](eval/reports/interview-question-routing-review.json).

Verification passed **310 PostgreSQL tests in 105.17 s**, the scoped isolated reset, **33/33 Chromium journeys in 43.18 s**, TypeScript/production/Docker builds, Ruff and Prettier. The API is healthy with its database preserved. The UI now says what was sent to the model, and rejected model output is labelled as a provider-response failure. Conservative routing and model interpretation still have the limits listed in the review; a successful route does not certify every generated fact.
...
Demonstrate in a small synthetic collection: state a project's backend owner/database; repeat it; explicitly change its database; ask about the latest value; inspect Memory history. A pure question should report no lasting fact. An unsupported project detail should remain unknown, while stable public knowledge can use the clearly labelled fallback. Name the project again when context is ambiguous. The learner sees at most six earlier user messages from the same conversation and collection; it never learns the assistant's reply. Clearing conversation or reloading starts a new local conversation context but does not delete saved messages.

Learning adds model latency. Failed learning preserves the original and does not prevent answering from it. Retry learning is explicit and bounded; an answer retry reuses the saved message and completed learning. A closed page or occupied lease can leave pending work visible in Memory, where Process sources resumes it. Private sends only the current input without capture, learned context or database writes. Full validation and live failures/successes are consolidated in [the conversation review](eval/reports/interview-conversation-review.json).

Final checks: **295 backend tests passed in 182.62 s; 32/32 browser journeys passed in 63.69 s**. Application/test/evaluator Ruff checks, formatting, TypeScript and production build passed. The isolated browser fixture reset passed without touching demo data. The final live collection **`interview-chat-20260922`** contains six saved user messages, two active memories and four claim revisions: Cedar's backend owner changed Mira → Ravi; its database changed PostgreSQL → SQLite. Repetition produced duplicate, and the unsupported launch question/general-knowledge request produced no_memory. All six final learning jobs succeeded with one extraction call each and one answer call per turn; measured round trips were 5.98–8.60 s. Initial failures remain in `chat-demo-20260922` and the report. Scope extraction is still imperfect: the prototype qualifier survived the original and answer but was omitted from the typed database-value text. This is a bounded demo, not a universal semantic-quality guarantee.
